...
def preprocess_data(data):
    data.replace("?", np.nan, inplace = True) # 替换问号为缺失值
    for col in categorical_columns:
        data[col] = data[col].fillna(data[col].mode()[0]) # 用众数替换缺失的地方
        data[col] = le.fit_transform(data[col]) # 对测试数据进行编码
    return data
...
# 模型训练
model.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False) # eval_set 减少过拟合的现象
